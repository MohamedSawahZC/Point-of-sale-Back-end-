...
namespace POSTeam
{
    class ReturnedInvoice
    {
        static BindingList<ReturnedInvoice> returnedInvoice;
        ...
        public static void Insert(Int64 Invoice_ID, Int64 Product_ID, float out_price, float in_price, float discount, float quantity)
        {
            ...
            Available_ID = DBConnection.getIDCounter("ReturnedInvoice");
            string[] names = { "ID", "Invoice_ID", "Product_ID", "Out_Price", "In_Price", "Discount", "Quantity" };
            string[] values = { ReturnedInvoice.Available_ID.ToString(), Invoice_ID.ToString(), Product_ID.ToString(), out_price.ToString(), in_price.ToString(), discount.ToString(), quantity.ToString() };
            try
            {
                DBConnection.insertRow("ReturnedInvoice", names, values);
            }
            catch (Exception e)
            {
                MessageBox.Show(e.Message);
            }
            DBConnection.setIDCounter("ReturnedInvoice", ReturnedInvoice.Available_ID + 1);
        }
